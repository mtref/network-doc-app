...
function SettingsPage({ showMessage }) {
  ...
  const fetchPdfTemplates = useCallback(async () => {
    try {
      // CORRECTED: Use api.get
      const data = await api.get('pdf_templates');
  ...
  const handleFileUpload = async (event, entityType) => {
    ...
    try {
      // CORRECTED: Use api.postForm for FormData
      const response = await api.postForm(`import/${entityType}`, formData);
  ...
  const handlePdfUpload = async (event) => {
    ...
    try {
      // CORRECTED: Use api.postForm for FormData
      await api.postForm('pdf_templates', formData);
  ...
  const handleDeletePdfTemplate = async (templateId) => {
    if (window.confirm('Are you sure you want to delete this PDF template?')) {
      try {
        // CORRECTED: Use api.delete
        await api.delete(`pdf_templates/${templateId}`);
  ...
  const handleSetDefaultPdf = async (templateId) => {
    try {
      // CORRECTED: Use api.post
      await api.post('app_settings/default_pdf', { default_pdf_id: templateId });
  ...
  const exportData = async (entityType) => {
    try {
        ...
        const response = await fetch(`${process.env.NODE_ENV === 'production' ? '' : 'http://localhost:5004'}/api/export/${entityType}`, {
            headers: {
        ...
        showMessage(`Error exporting data: ${error.message}`, 5000);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {user.role === 'Admin' && (
        <CollapsibleSection title="User Accounts" icon={<Users size={20} className="mr-2" />} isExpanded={isUserMgmtExpanded} setIsExpanded={setIsUserMgmtExpanded} bgColorClass="bg-indigo-50" hoverBgColorClass="hover:bg-indigo-100" textColorClass="text-indigo-700">
            <UserManagement showMessage={showMessage} />
        </CollapsibleSection>
      )}

      {user.role !== 'Viewer' && (
        <CollapsibleSection title="Import Data" icon={<Upload size={20} className="mr-2" />} isExpanded={isImportExpanded} setIsExpanded={setIsImportExpanded} bgColorClass="bg-blue-50" hoverBgColorClass="hover:bg-blue-100" textColorClass="text-blue-700">
      ...
      <CollapsibleSection title="Export Data" icon={<Download size={20} className="mr-2" />} isExpanded={isExportExpanded} setIsExpanded={setIsExportExpanded} bgColorClass="bg-green-50" hoverBgColorClass="hover:bg-green-100" textColorClass="text-green-700">
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {entityTypes.map(entity => (
      ...
      {user.role !== 'Viewer' && (
        <CollapsibleSection title="Manage PDF Templates" icon={<FileText size={20} className="mr-2" />} isExpanded={isPdfMgmtExpanded} setIsExpanded={setIsPdfMgmtExpanded} bgColorClass="bg-yellow-50" hoverBgColorClass="hover:bg-yellow-100" textColorClass="text-yellow-700">
      ...
      {user.role !== 'Viewer' && (
        <CollapsibleSection title="System Activity Log" icon={<Activity size={20} className="mr-2" />} isExpanded={isLogViewerExpanded} setIsExpanded={setIsLogViewerExpanded}>
