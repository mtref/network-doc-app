...
function SettingsPage({ showMessage }) {
  ...
  // State for collapsible sections, all default to false (collapsed)
  ...
  const handlePdfFileUpload = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/pdf_templates/upload`, { method: "POST", body: formData });
  ...
  const handleDeletePdfTemplate = async (pdfId) => {
    if (!window.confirm("Are you sure you want to delete this PDF template?")) return;
    try {
      const response = await fetch(`${API_BASE_URL}/pdf_templates/${pdfId}`, { method: "DELETE" });
      if (!response.ok) {
  ...
  const handleExport = async (entityType) => {
    try {
      ...
      showMessage(`Failed to export ${entityType} data: ${error.message}`, 8000);
    }
  };
  ...
  const handleImport = async (e) => {
    ...
    if (!importFile || !selectedImportEntityType) {
      showMessage("Please select a file and entity type for import.", 3000);
      return;
    }
    const formData = new FormData();
    formData.append("file", importFile);
    try {
      const response = await fetch(`${API_BASE_URL}/import/${selectedImportEntityType}`, { method: "POST", body: formData });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || `HTTP error! status: ${response.status}`);
      setImportStatus(result);
      showMessage(result.error_count > 0 ? `Import completed with ${result.error_count} errors.` : result.message || "Import successful!", 8000);
      setImportFile(null);
      setSelectedImportEntityType("");
      e.target.reset();
    } catch (error) {
      console.error("Error during import:", error);
      setImportStatus({ message: `Import failed: ${error.message}`, errors: error.details || [], error_count: 1, success_count: 0 });
  ...
  return (
    <div className="space-y-4 p-6 bg-white rounded-lg shadow-md border border-gray-200">
      ...
      >
        <p className="text-gray-700 mb-4">Select an entity type to export all its data to a CSV file.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {entityTypes.map((type) => (
            <button key={type.id} onClick={() => handleExport(type.id)} className="flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 transition-colors duration-200 text-sm font-medium">
      ...
      >
        <p className="text-gray-700 mb-4">Upload a CSV file to import data. Ensure the CSV format matches the exported format.</p>
        <form onSubmit={handleImport} className="space-y-4">
          ...
          <div>
            <label htmlFor="import-entity-type" className="block text-sm font-medium text-gray-700 mb-1">
              Select Entity Type:
            </label>
            <select
