...
// UPDATED: Replaced the deprecated `document.execCommand` with the modern, async Clipboard API.
...
const PasswordManager = ({ showMessage }) => {
  ...
  const handleEditClick = async (entry) => {
    try {
      const fullEntry = await api.get(`passwords/${entry.id}`);
  ...
  const togglePasswordVisibility = async (entryId) => {
    ...
    try {
      const entry = await api.get(`passwords/${entryId}`);
  ...
  const copyToClipboard = async (text) => {
    ...
    try {
      ...
      showMessage("Failed to copy text.", 3000);
            ...
            {filteredEntries.map((entry) => (
              <tr key={entry.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                  <button onClick={() => handleEditClick(entry)} className="text-indigo-600 hover:text-indigo-900" title="Edit Entry"><Edit size={18} /></button>
                  <button onClick={() => handleDeleteClick(entry.id)} className="text-red-600 hover:text-red-900" title="Delete Entry"><Trash2 size={18} /></button>
                </td>
              </tr>
            ))}
          </tbody>
