...
// UPDATED: API calls now use the correct methods from the refactored api.js service.
...
function SystemLogViewer({ showMessage }) {
  ...
  const fetchLogs = useCallback(async (page = 1, perPage = 15) => {
    ...
    try {
      // CORRECTED: Changed api(...) to api.get(...)
  ...
  const handleRevert = async (logId) => {
    if(window.confirm('Are you sure you want to revert this action? This can have cascading effects.')) {
        try {
            // CORRECTED: Changed api(...) to api.post(...)
            const response = await api.post(`logs/${logId}/revert`);
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md border border-gray-200">
      ...
      <div className="flex justify-between items-center mt-4 text-sm">
        <span>Page {pagination.current_page || 1} of {pagination.pages || 1} (Total: {pagination.total || 0} logs)</span>
