...
# UPDATED: This is a consolidated version containing all required endpoints, including the missing /logs route.
...
from .models import User, Location, Rack, PC, PatchPanel, Switch, Connection, ConnectionHop, PdfTemplate, AppSettings, PasswordEntry, StockItem
...
def register_routes(app):
    ...
    # --- Stock Management ---
    @app.route('/stock_items', methods=['GET', 'POST'])
    @jwt_required()
    def handle_stock_items():
        if request.method == 'POST':
            ...
            def post_logic():
                data = request.json
                try:
                    new_item = StockItemService.create(data, action_by=get_jwt_identity())
                    return jsonify(new_item.to_dict()), 201
                except IntegrityError:
                    db.session.rollback()
                    return jsonify({'error': 'A stock item with this serial number already exists.'}), 409
                except Exception as e:
                    db.session.rollback()
                    return jsonify({'error': str(e)}), 500
            return post_logic()
        else: # GET
            items = StockItemService.get_all_stock_items()
            return jsonify([item.to_dict() for item in items])

    @app.route('/stock_items/<int:item_id>', methods=['GET', 'PUT', 'DELETE'])
    @jwt_required()
    def handle_stock_item_by_id(item_id):
        item = StockItemService.get_by_id(item_id)
        if not item:
        ...
        def editor_logic():
            if request.method == 'PUT':
                data = request.json
                try:
                    updated_item = StockItemService.update(item, data, action_by=get_jwt_identity())
                ...
                StockItemService.delete(item, action_by=get_jwt_identity())
                return jsonify({'message': 'Stock item deleted successfully'})
        return editor_logic()

    # --- Locations, Racks, PCs, Switches, Patch Panels, Connections ---
    ...
    def handle_pdf_templates():
        if request.method == 'POST':
            ...
            def post_logic():
                ...
                try:
                    new_pdf = PdfTemplateService.upload_pdf_template(file, current_app.config, action_by=get_jwt_identity())
