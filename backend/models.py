...
# UPDATED: Added a new StockItem model for managing inventory and resources.
...
# --- New Model for Stock Management ---
class StockItem(db.Model):
    __tablename__ = 'stock_items'
    id = db.Column(db.Integer, primary_key=True)
    item_type = db.Column(db.String(100), nullable=False) # e.g., 'PC', 'Monitor', 'HDD', 'SSD', 'RAM', 'Keyboard', 'Mouse'
    name = db.Column(db.String(255), nullable=False) # e.g., 'Dell OptiPlex 7070', 'Samsung 24" Odyssey', '1TB Seagate Barracuda'
    serial_number = db.Column(db.String(255), nullable=True, unique=True)
    status = db.Column(db.String(50), nullable=False, default='In Stock') # 'In Stock', 'In Use', 'Defective', 'Retired'
    quantity = db.Column(db.Integer, nullable=False, default=1)
    notes = db.Column(db.Text, nullable=True)
    purchase_date = db.Column(db.Date, nullable=True)
    
    # Optional: Link a stock item to a specific PC if it's 'In Use'
    assigned_to_pc_id = db.Column(db.Integer, db.ForeignKey('pcs.id'), nullable=True)
    assigned_to_pc = db.relationship('PC', backref='stock_items')

    def to_dict(self):
        """Converts a StockItem object to a dictionary."""
        return {
            'id': self.id,
            ...
            'assigned_to_pc_id': self.assigned_to_pc_id,
            'assigned_to_pc_name': self.assigned_to_pc.name if self.assigned_to_pc else None
        }
