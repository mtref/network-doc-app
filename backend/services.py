...
class SystemLogService:
    ...
    def revert_log_action(log_id, action_by='system'):
        ...
        service_map = {'Location': LocationService, 'Rack': RackService, 'PC': PCService, 'Patch Panel': PatchPanelService, 'Switch': SwitchService, 'Connection': ConnectionService}
        ...
        if log_to_revert.action_type == 'CREATE':
            ...
            if entity:
                service.delete(entity, action_by=action_by)
            else:
                raise ValueError(f"{log_to_revert.entity_type} with ID {log_to_revert.entity_id} not found for deletion.")
        elif log_to_revert.action_type == 'DELETE':
            if not log_to_revert.details:
            ...
            entity = service.get_by_id(log_to_revert.entity_id)
            if not entity:
                raise ValueError(f"{log_to_revert.entity_type} with ID {log_to_revert.entity_id} not found for update.")
...
class LocationService:
    ...
    def update(location, data, is_revert=False, action_by='system'):
        if not is_revert:
            ...
            if changes:
                SystemLogService.create_log('UPDATE', 'Location', location.id, data.get('name', location.name), changes, action_by=action_by)
